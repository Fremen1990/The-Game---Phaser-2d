...
class Game extends Phaser.Scene {
  ...
  addMap () {
    ...
    const backgroundLayer_1 = this.map.createStaticLayer(
      'Background-1',
      backgroundTiles
    )
    const backgroundLayer_2 = this.map.createStaticLayer(
      'Background-2',
      backgroundTiles
    )
    ...
    this.coinsFoodGroup = this.physics.add.group({
      immovable: false,
      allowGravity: false
    })
  ...
  update (time, delta) {
    ...
    if (this.hero.isDead() && this.hero.getBounds().top > cameraBottom + 500) {
      this.hero.destroy()
      this.addHero()
    }
  }
